...
int main() {
	...
	*/
	...
	Data_conf output13_tiled_conf = {output13_conf.h/h_num_tiles, output13_conf.w/w_num_tiles, output13_conf.c};
	Data_conf input13_tiled_conf = {output13_tiled_conf.h * pool1_conf.h, output13_tiled_conf.h * pool1_conf.w, input13_conf.c};
	...
	Data_conf input11_tiled_conf = {output11_tiled_conf.h + (conv11_conf.h - 1), output11_tiled_conf.h + (conv12_conf.w - 1), input11_conf.c};

	float *input11_tiled = (float *)mkl_calloc(input11_tiled_conf.h * input11_tiled_conf.w * input11_tiled_conf.c,
									bytes, alignment);
	float *output11_tiled = (float *)mkl_calloc(output11_tiled_conf.h * output11_tiled_conf.w * output11_tiled_conf.c,
									bytes, alignment);
	float *output12_tiled = (float *)mkl_calloc(output12_tiled_conf.h * output12_tiled_conf.w * output12_tiled_conf.c,
									bytes, alignment);
	float *output13_tiled = (float *)mkl_calloc(output13_tiled_conf.h * output13_tiled_conf.w * output13_tiled_conf.c,
									bytes, alignment);
	...
	vector<Layer_conf> g1_layers;

	Layer_conf l11_conf = {conv, 3, 3, 64, 1, 1};
	Layer_conf l12_conf = {conv, 3, 3, 64, 1, 1};
	Layer_conf l13_conf = {pool, 2, 2, 64, 2, 0};

	g1_layers.push_back(l11_conf);
	g1_layers.push_back(l12_conf);
	g1_layers.push_back(l13_conf);

	// layer_g1.push_back(std::make_pair(, ));
	
	std::vector<vector<Data_conf>> conf_list = create_tiled_conf(input_conf, g1_layers, h_num_tiles);

	// std::vector<vector<PAD_CONF>> pad_conf;

	float *temp_patch;
	for (int h_idx = 0; h_idx < h_num_tiles; h_idx++) {
		for (int w_idx = 0; w_idx < w_num_tiles; w_idx++) {
			//load input
			TILE_IDX tile_idx = {h_idx, w_idx};

			// load_tile(input11, input11_conf, tile_idx, input11_tiled, input11_tiled_conf);
			// temp_patch = (float *)mkl_calloc(input11_tiled_conf);
			Data_conf input11_tiled_conf = get_tile_conf(tile_idx, num_tiles, );

			PAD_CONF pad = get_pad_conf(tile_idx, num_tiles);

			alloc_patch(temp_patch, output11_conf, h_num_tiles, conv11_conf);
			conv_im2row(input11_tiled, output11_tiled, conv11_weights, conv11_biases, conv11_conf, input11_tiled_conf, output11_tiled_conf);
	
			mkl_free(temp_patch);
			alloc_patch(temp_patch, output12_conf, h_num_tiles, conv12_conf);
			conv_im2row(output11_tiled, output12_tiled, conv12_weights, conv12_biases, conv12_conf, input12_tiled_conf, output12_tiled_conf);
			
			mkl_free(temp_patch);
			pool_forward(output12_tiled, output13_tiled, input13_tiled_conf, output13_tiled_conf, pool1_conf);
			save_tile(output13_tiled, output13_tiled_conf, tile_idx, output13, output13_conf);
			//save output
		}
	}
