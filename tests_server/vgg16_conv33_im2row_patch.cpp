...
int main() {
	...
	if (tiled) {
		...
		for (int i = 0; i < times; i++) {
			...
			for (int h_tile = 0; h_tile < h_num_tiles; h_tile++) {
				for (int w_tile = 0; w_tile < w_num_tiles; w_tile++) {
					...
					patch_ret(input33_tiled, output33_tiled, conv33_weights, conv33_biases, conv33_tiled_conf,
		...
		for (int i = 0; i < times; i++) {
			...
			// for (int idx = 0; idx < output33_conf.h * output33_conf.w * output33_conf.c; idx++) {
			// 	output33[idx] = 0.0f;
			// }
			start = std::chrono::system_clock::now();
			patch_ret(input33, output33, conv33_weights,conv33_biases, conv33_conf, input33_conf, output33_conf);
			end = std::chrono::system_clock::now();
			std::chrono::duration<double> elapsed_time = end-start;
			tot_time += elapsed_time.count();
			// cout<<elapsed_time.count()<<endl;
