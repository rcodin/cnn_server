...
int main() {
	...
	int w_num_tiles = 7;
		...
		Conv_conf conv43_tiled_conf = {3, 3, 1, 0};
		Data_conf input43_tiled_conf = {input43_conf.h/h_num_tiles + (conv43_conf.h - 1),
						input43_conf.w/w_num_tiles  + (conv43_conf.w - 1), input43_conf.c};
		Data_conf output43_tiled_conf = {output43_conf.h/h_num_tiles, output43_conf.w/w_num_tiles, output43_conf.c};


		float *input43 = output42;

		float *output43 = (float *)mkl_calloc(output43_conf.h * output43_conf.w *
			output43_conf.c, bytes, alignment);

		float *input43_tiled = (float *)mkl_calloc(input43_tiled_conf.h * input43_tiled_conf.w *
			input43_tiled_conf.c, bytes, alignment);

		float *output43_tiled = (float *)mkl_calloc(output43_tiled_conf.h * output43_tiled_conf.w *
			output43_tiled_conf.c, bytes, alignment);
		...
		float *input44 = output43;
		float *output44 = (float *)mkl_calloc(output44_conf.h * output44_conf.w *
			output44_conf.c, bytes, alignment);
		...
		Conv_conf conv51_tiled_conf = {3, 3, 1, 0};
		Data_conf input51_tiled_conf = {input51_conf.h/h_num_tiles + (conv51_conf.h - 1),
						input51_conf.w/w_num_tiles  + (conv51_conf.w - 1), input51_conf.c};
		Data_conf output51_tiled_conf = {output51_conf.h/h_num_tiles, output51_conf.w/w_num_tiles, output51_conf.c};


		float *input51 = output44;

		float *output51 = (float *)mkl_calloc(output51_conf.h * output51_conf.w *
			output51_conf.c, bytes, alignment);

		float *input51_tiled = (float *)mkl_calloc(input51_tiled_conf.h * input51_tiled_conf.w *
			input51_tiled_conf.c, bytes, alignment);

		float *output51_tiled = (float *)mkl_calloc(output51_tiled_conf.h * output51_tiled_conf.w *
			output51_tiled_conf.c, bytes, alignment);
		...
		float *conv43_weights;
		float *conv43_biases;

		float *conv51_weights;
		float *conv51_biases;
	...
	if (tiled) {
		...
		for (int h_tile = 0; h_tile < h_num_tiles; h_tile++) {
			for (int w_tile = 0; w_tile < w_num_tiles; w_tile++) {
				...
				int h_base = h_tile * (input43_tiled_conf.h - (conv43_conf.h - 1));
				int w_base = w_tile * (input43_tiled_conf.w - (conv43_conf.w - 1));

				TILE_BASE tile_base = {h_base, w_base};

				load_tile(input43, input43_conf, tile_base, h_num_tiles, 
							input43_tiled, input43_tiled_conf);

				conv_im2row(input43_tiled, output43_tiled, conv43_weights, conv43_biases, conv43_tiled_conf,
					input43_tiled_conf, output43_tiled_conf);

				save_tile(output43_tiled, output43_tiled_conf, tile_base, output43, output43_conf);

			}
		}


		pool_forward(input44, output44, input44_conf, output44_conf, pool2_conf);
		...
		for (int i = 0; i < times; i++) {
			...
			for (int h_tile = 0; h_tile < h_num_tiles; h_tile++) {
				for (int w_tile = 0; w_tile < w_num_tiles; w_tile++) {
					...
					int h_base = h_tile * (input51_tiled_conf.h - (conv51_conf.h - 1));
					int w_base = w_tile * (input51_tiled_conf.w - (conv51_conf.w - 1));

					TILE_BASE tile_base = {h_base, w_base};

					load_tile(input51, input51_conf, tile_base, h_num_tiles, 
								input51_tiled, input51_tiled_conf);

					conv_im2row(input51_tiled, output51_tiled, conv51_weights, conv51_biases, conv51_tiled_conf,
						input51_tiled_conf, output51_tiled_conf);

					save_tile(output51_tiled, output51_tiled_conf, tile_base, output51, output51_conf);
		...
		conv_im2row(input43, output43, conv43_weights,conv43_biases, conv43_conf, input43_conf, output43_conf);

		pool_forward(input44, output44, input44_conf, output44_conf, pool2_conf);

		for (int i = 0; i < times; i++) {
			
			for (int idx = 0; idx < output51_conf.h * output51_conf.w * output51_conf.c; idx++) {
				output51[idx] = 0.0f;
			}
			start = std::chrono::system_clock::now();
			conv_im2row(input51, output51, conv51_weights,conv51_biases, conv51_conf, input51_conf, output51_conf);
			end = std::chrono::system_clock::now();
			std::chrono::duration<double> elapsed_time = end-start;
			tot_time += elapsed_time.count();
			// cout<<elapsed_time.count()<<endl;
	...
	// for (int i = 0; i < output51_conf.h ; i++) {
	// 	for (int j = 0; j < output51_conf.w; j++) {
	// 		for (int k = 0; k < output51_conf.c; k++) {
	// 			int idx = (i * output51_conf.w + j) * output51_conf.c + k;
	// 			cout<<fixed<<setprecision(10)<<output51[idx]<<endl;
	// 		}
	// 	}
	// }
}