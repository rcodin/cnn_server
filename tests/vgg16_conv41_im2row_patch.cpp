...
int main() {
	...
	if (tiled) {
		...
		for (int i = 0; i < times; i++) {
			...
			for (int h_tile = 0; h_tile < h_num_tiles; h_tile++) {
				for (int w_tile = 0; w_tile < w_num_tiles; w_tile++) {
					...
					patch_ret(input41_tiled, output41_tiled, conv41_weights, conv41_biases, conv41_tiled_conf,
		...
		for (int i = 0; i < times; i++) {
			...
			for (int idx = 0; idx < output41_conf.h * output41_conf.w * output41_conf.c; idx++) {
				output41[idx] = 0.0f;
			}
			start = std::chrono::system_clock::now();
			patch_ret(input41, output41, conv41_weights,conv41_biases, conv41_conf, input41_conf, output41_conf);
			end = std::chrono::system_clock::now();
			std::chrono::duration<double> elapsed_time = end-start;
			tot_time += elapsed_time.count();
			// cout<<elapsed_time.count()<<endl;
