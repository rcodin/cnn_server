...
int main() {
	...
	if (tiled) {
		...
		for (int i = 0; i < times; i++) {
			...
			for (int h_tile = 0; h_tile < h_num_tiles; h_tile++) {
				for (int w_tile = 0; w_tile < w_num_tiles; w_tile++) {
					...
					patch_ret(input43_tiled, output43_tiled, conv43_weights, conv43_biases, conv43_tiled_conf,
		...
		for (int i = 0; i < times; i++) {
			...
			start = std::chrono::system_clock::now();
			patch_ret(input43, output43, conv43_weights,conv43_biases, conv43_conf, input43_conf, output43_conf);
			end = std::chrono::system_clock::now();
			std::chrono::duration<double> elapsed_time = end-start;
			tot_time += elapsed_time.count();
			// cout<<elapsed_time.count()<<endl;
