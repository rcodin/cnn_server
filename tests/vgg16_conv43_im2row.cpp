...
int main() {
	...
	if (tiled) {
		...
		for (int i = 0; i < times; i++) {
			...
			for (int idx = 0; idx < output43_conf.h * output43_conf.w * output43_conf.c; idx++) {
				output43[idx] = 0.0f;
			}
			start = std::chrono::system_clock::now();
	...
	// for (int i = 0; i < output43_conf.h ; i++) {
	// 	for (int j = 0; j < output43_conf.w; j++) {
	// 		for (int k = 0; k < output43_conf.c; k++) {
	// 			int idx = (i * output43_conf.w + j) * output43_conf.c + k;
	// 			cout<<fixed<<setprecision(10)<<output43[idx]<<endl;
	// 		}
	// 	}
	// }
}